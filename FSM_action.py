...
def update_log_state():
    ...
    for log_line_container in log_container.message_list:
        if log_line_container.line_type == "Create Game":
            InBattle = True
        # if log_line_container.line_type == "Create Game":
        #     InBattle = False
    ...
    while True:
        ...
        if log_state.is_end:
            ...
            if log_state.my_entity.query_tag("PLAYSTATE") == "WON":
                ...
                lost_battle_counter += 1
                info_print("你输了")
            battle_counter = won_battle_counter + lost_battle_counter
            if battle_counter % 5 == 0:
                result = 'Lost'
                if win_flag: result = 'Won'
                ...
                SMS.sendMessage('[AutoHS][%d th Battle finished][win=%d][lost=%d][time=%s]' % (battle_counter, won_battle_counter, lost_battle_counter, cur_time))
